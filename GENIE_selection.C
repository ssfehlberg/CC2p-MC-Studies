...
void GENIE_selection::Loop(){
  ...
  /////////////////////////////////////
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     if(_debug) std::cout<<"[END OF THE CUTS] Number of Final State particles: "<<nf<<std::endl;

     TVector3 particle_mom;
     for(int i=0; i < nf; i++){
       particle_mom.SetXYZ(pxf[i],pyf[i],pzf[i]);
       if(_debug) std::cout<<"[END OF THE CUTS] PDF of the Particles: "<<pdgf[i]<<std::endl;
       if(_debug) std::cout<<"[END OF THE CUTS] momentum magnitude of the particle: "<<particle_mom.Mag()<<std::endl;
       if(_debug) std::cout<<"[END OF THE CUTS] rescattering  value: "<<resc[i]<<std::endl;
     }
     ...
     if(res == true){
       if(_debug) std::cout<<"Number of final state particles: "<<nf<<std::endl;
       for(int i=0; i < nf; i++){
	 TVector3 particle_mom(pxf[i],pyf[i],pzf[i]);
	 int pdg = pdgf[i];
	 if(std::abs(pdg) == 211 || pdg == 111){
	   if(_debug) std::cout<<"FUCK ME!"<<std::endl;
	 }
	 if(_debug) std::cout<<Form("Particle %d PDG: ",i)<<pdg<<", Momentum: "<<particle_mom.Mag()<<std::endl;
       }
       res0++;
     }
     if(dis == true){
